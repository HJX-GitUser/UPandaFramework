...
public class Reporter : MonoBehaviour
{
    ...
    /// <summary>日志条数上限：超过后按批丢弃最旧的日志（0 = 不限）。避免长时间运行内存持续增长。</summary>
    ...
    void AddLog(string condition, string stacktrace, LogType type)
    {
        ...
        logs.Add(log);
        // 修复：按条数上限丢弃最旧的日志（原实现只在总内存超过 maxSize 时整体 Clear，会一性丢掉全部历史）
    ...
    void OnApplicationQuit()
    {
        ...
        //layerPrefs.SetInt("Reporter_show", (show == true) ? 1 : 0);
