...
namespace UPandaGF.RunTime.InteractiveTaskScoringSystem
{
    ...
    public abstract class TaskEntityBase : MonoBehaviour, InteractiveTrigger
    {
        ...
        protected virtual void Awake()
        {
            ...
            // 修复：不再自动挂载示例触发器，请显式挂载 TaskTriggerExample（或自行实现 InteractiveTrigger 触发器）
        }
        ...
        public virtual void OnSelect()
        {
            ...
            ITaskHost host = TaskDataManager.Instance;   // 通过接口访问任务宿主（便于替换/测试/多实例）
            if (host == null)
            {
                Debug.LogError("场景中缺少 TaskDataManager，无法进行任务检查");
                return;
            }
